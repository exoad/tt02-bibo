...
REM   tests\build_editor_test.bat [run]   - compile, optionally run
REM editor.cxx is ImGui-free by design, so vim bindings, auto-closing braces and
REM auto-indent are checkable without a window. syntax.cxx includes imgui.h only
REM for IM_COL32, hence the include path below.
