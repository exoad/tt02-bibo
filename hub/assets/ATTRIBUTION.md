...
The pack's own licence text ships verbatim as `assets/models/LICENSE-kenney.txt`.
...
**chromatic** swatch (the greys that dominate are the underbody, which is never
...
TT-02 - a stand-in with the right footprint and the right colour, nothing more.
...
questions that are separate from whatever copyright licence the uploader chose.
...
**Attribution is a condition of the licence, not a courtesy.** It has to appear
...
author offers is buying a royalty-free licence, which removes the requirement.
...
## The application icon — `assets/tt02.ico`

**Not** from Fugue. Generated by `assets/make_icon.ps1` from primitives, in the
app's own visual language, so it carries no third-party licence. Fugue is 16×16
only; an application icon needs 256.
