...
Bool exemptLine(const Str& line)
{
    ...
    // `using Float32 = float;` is the alias DEFINITION - shared.hpp has to name
...
Void checkLine(const Str& raw, const Str& code, Int32 lineNo, Lang lang,
               Vec<diag::Item>& out)
{
    if(exemptLine(raw))
    {
    ...
    if(lang == Lang::LANG_CPP)
    {
        for(const Char* t : STD_TYPES)
        {
            ...
            {
                add(out, lineNo, pos,
                    "unaliased std type - use the shared.hpp alias "
                    "(Vec, Str, Map, Mutex, ...)");
                break;
            }
        }
