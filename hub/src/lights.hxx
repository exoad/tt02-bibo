...
namespace lights
{
  ...
  static_assert(BLINK_ON_S + BLINK_OFF_S >= 0.500, "flash rate over 120/min");
  static_assert(BLINK_ON_S + BLINK_OFF_S <= 0.666, "flash rate under 90/min");
  static_assert(BLINK_ON_S >= BLINK_OFF_S, "on-time shorter than off-time");
  ...
  [[nodiscard]] Input detect(
      AutoState& st,
      const Drive& d,
      Float64 seconds,
      const AutoConfig& cfg = AutoConfig{}
  ) noexcept;
