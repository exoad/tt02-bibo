...
// ---------------------------------------------------------------------------
// Pure geometry. No ImGui, and tested in tests/test_workspace.cxx.
// ---------------------------------------------------------------------------
...
Void toCanvas(Float32 sx, Float32 sy, const Canvas& c, Float32 originX, Float32 originY, Float32& cx, Float32& cy);
...
[[nodiscard]] Int32 hitTest(const Panel* panels, Int32 count, const Canvas& c, Float32 originX, Float32 originY, const Float32* screenH, Float32 sx, Float32 sy);
...
Void zoomAt(Canvas& c, Float32 factor, Float32 sx, Float32 sy, Float32 originX, Float32 originY);

// Zoom and pan that bring every open panel into `viewW` x `viewH`. Leaves the
// canvas untouched when nothing is open.
Void fitAll(const Panel* panels, Int32 count, Canvas& c, Float32 viewW, Float32 viewH);
