...
void ApplyStyle(float dpi_scale)
{
    ...
    // ------------------------------------------------------------ geometry
    // Deliberately NOTHING here. Every metric is stock ImGui: square corners,
    // stock padding, stock borders, stock scrollbar. The only departure from a
    // default ImGui app is the font size, which is set in LoadFonts().
    //
    // Earlier revisions softened the corners and opened up the spacing to suit
    // the larger type. That read as a themed app rather than an ImGui one, so
    // it is gone - the sizes below come straight from ImGuiStyle()'s defaults.

    // Convert the stock logical sizes into physical px, exactly once.
