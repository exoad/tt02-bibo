...
namespace link
{
  ...
  Opt<PoseSeen> Session::poseSeen(Int64 nowMs) const
  {
      if(!havePose)
      {
          return {};
      }
      const Int64 age = ageOf(*this, nowMs, poseAtMs, pose.tMonoUs);
      if(age > GONE_MS)
      {
          return {};
      }
      ...
      p.pose = pose;
      p.ageMs = age;
      p.stale = age > staleBandMs(poseRate);
      return p;
  }
  ...
  Void ingestFrame(Session& s, const bibowire::Frame& f, Int64 nowMs)
  {
      ...
      {
          bibowire::Pose m;
          if(!bibowire::readPose(f.body, ver, &m))
          {
              ++s.refusedFrames;
              return;
          }
  ...
  UInt32 subscriptionMask(Bool withCamera)
  {
      ...
      UInt32 mask = bibowire::typeBit(bibowire::Type::TYPE_SCAN)
                    ...
                    | bibowire::typeBit(bibowire::Type::TYPE_POSE);
      if(withCamera)
      {
