/*
 * Bringing up the SPI colour display, drawn with gfx.h.
 *
 * A named sketch, so it survives. firmware/src/sketch.c is a scratch SLOT that
 * the hub overwrites with whichever sketch is open when you press Build &
 * Flash - which is what it is for, and why nothing you want to keep lives
 * there.
 *
 * Flash this one FIRST. 13-display-gfx.c is the interesting one, but until the
 * bars below appear there is nothing in it that can tell you why a blank screen
 * is blank.
 *
 *   Display   Pico          why
 *   GND       GND
