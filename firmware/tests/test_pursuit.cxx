...
/**
 * @brief Runs every geometry, kinematics, and pursuit check and reports the pass/fail count.
 *
...
Int32 main(Void)
{
    ...
    check(
        near(geom::wrapPi(GEOM_PI + 0.1f), -GEOM_PI + 0.1f, 1e-4f),
        "just past pi wraps to just past -pi"
    );
    check(near(geom::wrapPi(-GEOM_PI - 0.1f), GEOM_PI - 0.1f, 1e-4f), "and the other way");
    check(
        near(geom::wrapPi(20.0f * GEOM_TAU + 0.3f), 0.3f, 1e-3f),
    ...
    check(geom::toLocal(at0, leftOfCar).y > 0.0f, "a point to the left is +y in the car frame");

    geom::Pose facingY;
    facingY.heading = GEOM_PI / 2.0f;
    const geom::Vec2 ahead = geom::toLocal(facingY, geom::Vec2{ 0.0f, 1.0f });
    check(
        near(ahead.x, 1.0f, 1e-4f) && near(ahead.y, 0.0f, 1e-4f),
    ...
    check(
        near(back.x, there.x, 1e-3f) && near(back.y, there.y, 1e-3f),
        "toLocal and toWorld are inverses"
    );

    printf("\nkinematics\n\n");

    check(near(kin::curvatureFor(0.0f, 0.257f), 0.0f, 1e-6f), "straight wheels is zero curvature");
    check(kin::curvatureFor(0.3f, 0.257f) > 0.0f, "a left steer is a positive curvature");

    const Float32 k = kin::curvatureFor(0.3f, 0.257f);
    check(near(kin::steerFor(k, 0.257f), 0.3f, 1e-4f), "steerFor and curvatureFor are inverses");

    check(
        near(kin::curvatureFor(0.3f, 0.0f), 0.0f, 1e-6f),
        "a zero wheelbase is 0, not a division by zero"
    );

    check(
        near(kin::steerFraction(KIN_MAX_STEER_RAD, KIN_MAX_STEER_RAD), 1.0f, 1e-4f),
        "full lock is a fraction of 1"
    );
    check(
        near(kin::steerFraction(2.0f, KIN_MAX_STEER_RAD), 1.0f, 1e-4f),
    ...
    check(
        near(p.x, 1.0f, 1e-4f) && near(p.y, 0.0f, 1e-4f),
    ...
    check(
        near(c.x, 0.0f, 0.02f) && near(c.y, 0.0f, 0.02f),
        "a full circle comes back to where it started"
    );

    check(
        kin::minTurnRadius(KIN_MAX_STEER_RAD, 0.257f) > 0.0f,
    ...
    check(l.valid && l.curvature > 0.0f, "a car right of the path steers LEFT to rejoin it");
    ...
    check(stopped.lookahead >= f.minM, "at rest the lookahead is the floor, not zero");
    check(
        !isnan(stopped.curvature) && !isinf(stopped.curvature),
    ...
    pursuit::reset(&f);
    check(
        pursuit::follow(&f, &path, on, 50.0f).lookahead <= f.maxM + 1e-4f,
    ...
    pursuit::Path empty;
    check(
        !pursuit::follow(&f, &empty, on, 1.0f).valid,
