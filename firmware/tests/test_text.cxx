/*
 * The parsers in lib/text.hxx.    tools\test.bat text run
 *
 * The cases that matter: "0" parses as 0, "12abc" fails where atoi() says 12,
 * "" and "banana" fail without touching the caller's variable, and starts()
 * refuses a prefix longer than the string. Host-compiled: text.hxx needs nothing
 * from the Pico SDK, and keeping it that way keeps the parsers testable.
 */
...
static Void testTwoInts(Void)
{
    ...
    /* sscanf("%d %d") accepts these and stops looking. */
