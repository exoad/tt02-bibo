// Material Symbols, the standard 24 px paths, inlined: the phone on the
// hotspot has no internet, so nothing is fetched - not a font, not a sprite.
// icon(name) returns the SVG markup; the fill is currentColor so a tone class
// on the parent colours it like the text beside it.
...
const PATHS = {
  ...
  view_in_ar: 'M18.25 7.6l-5.5-3.18c-.46-.27-1.04-.27-1.5 0L5.75 7.6c-.46.27-.75.76-.75 1.3v6.35c0 .54.29 1.03.75 1.3l5.5 3.18c.46.27 1.04.27 1.5 0l5.5-3.18c.46-.27.75-.76.75-1.3V8.9c0-.54-.29-1.03-.75-1.3zM7 14.96v-4.62l4 2.32v4.61l-4-2.31zm5-4.03L8 8.61l4-2.31 4 2.31-4 2.32zm1 6.34v-4.61l4-2.32v4.62l-4 2.31zM7 2H3.5C2.67 2 2 2.67 2 3.5V7h2V4h3V2zm10 0h3.5c.83 0 1.5.67 1.5 1.5V7h-2V4h-3V2zM7 22H3.5c-.83 0-1.5-.67-1.5-1.5V17h2v3h3v2zm10 0h3.5c.83 0 1.5-.67 1.5-1.5V17h-2v3h-3v2z',
  ...
  developer_board: 'M22 9V7h-2V5c0-1.1-.9-2-2-2H4c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2v-2h2v-2h-2v-2h2v-2h-2V9h2zm-4 10H4V5h14v14zM6 13h5v4H6zm6-6h4v3h-4zM6 7h5v5H6zm6 4h4v6h-4z',
...
export function icon(name, size) {
  ...
  if (!d) return '';
  const s = size || 24;
  return '<svg class="icon" width="' + s + '" height="' + s + '" viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path fill="currentColor" d="' + d + '"/></svg>';
}

// Fills every <i data-icon="name"> in the document with its SVG, once, at load.
export function mount(root) {
  (root || document).querySelectorAll('[data-icon]').forEach(function (n) {
    n.innerHTML = icon(n.dataset.icon, Number(n.dataset.size) || 24);
  });
}
