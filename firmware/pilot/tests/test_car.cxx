...
//
// Not proved here: the threads, arming and the signal hooks against a real lidar
// and Pico. That needs a run on the car, on a stand.
//
// The ports below do not exist, so even a build with the lidar SDK on the board
// never touches the real lidar or Pico.
#include "shared.hxx"
...
// sum after the call stops a compiler turning the recursion into a loop.
...
Int32 main()
{
    ...
    }
    {
        ...
        if(master >= 0)
        {
            ...
            // A child inherits the crash hooks the Cars above installed: overflowing
            // its stack still writes STOP, then it dies of the SIGSEGV.
