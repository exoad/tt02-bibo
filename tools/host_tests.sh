...
#
# Exits 1 if any suite fails to build, pass or parse, judged by exit codes, not
# output. Only chassis reaches into hal.hxx, so only it gets BIBO_FAKE_HAL, which
# swaps the SDK half for tests/fakes/hal.hxx.
set -u
...
for src in firmware/tests/test_*.cxx; do
    ...
    # tidy's report is stdout, for the caller to count; its stderr chatter is dropped.
    if [ "$mode" = tidy ]; then
